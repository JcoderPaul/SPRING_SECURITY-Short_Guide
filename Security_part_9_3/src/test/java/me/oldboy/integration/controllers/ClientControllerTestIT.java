...
class ClientControllerTestIT extends TestContainerInit {
    ...
    private ClientAuthRequest testClientAuthRequest, notValidAuthRequest, notExistingAuthRequest;
    ...
    void setUp(){
        ...
        /* Аутентификация верными данными */
        testClientAuthRequest = ClientAuthRequest.builder()
                .username(EXIST_EMAIL)
                .password(TEST_PASS)
                .build();
        notValidAuthRequest = ClientAuthRequest.builder()
                .username("wer")
                .password("1")
                .build();
        notExistingAuthRequest = ClientAuthRequest.builder()
                .username(TEST_EMAIL)
                .password(TEST_PASS)
                .build();
    }
    ...
    void registrationClient_ValidRegData_ShouldReturnOk_AndSavedClientDto_Test() {
        ...
        assertAll(
                ...
                () -> assertThat(savedClientDto.getClientName()).isEqualTo(testClientCreateDto.getDetails().getClientName()),
                () -> assertThat(savedClientDto.getClientSurName()).isEqualTo(testClientCreateDto.getDetails().getClientSurName()),
                () -> assertThat(savedClientDto.getAge()).isEqualTo(testClientCreateDto.getDetails().getAge())
        );
    ...
    @Test
    @SneakyThrows
    void loginClient_ShouldReturnCorrectClientAuthResponse_Test() {
        String requestBodyData = objectMapper.writeValueAsString(testClientAuthRequest);

        MvcResult result = mockMvc.perform(post("/api/loginClient")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBodyData))
                .andExpect(status().isOk())
                .andReturn();
        ...
        ClientAuthResponse clientAuthResponse = objectMapper.readValue(strReturn, ClientAuthResponse.class);
        assertAll(
                () -> assertThat(strReturn.contains("id")).isTrue(),
                () -> assertThat(strReturn.contains("clientLogin")).isTrue(),
                () -> assertThat(strReturn.contains("accessToken")).isTrue(),
                () -> assertThat(clientAuthResponse.getClientLogin()).isEqualTo(testClientAuthRequest.getUsername())
        );
    }

    @Test
    @SneakyThrows
    void loginClient_NotValidData_ShouldReturnBadRequest_AndErrorWarnings_Test() {
        ...
        assertAll(
                () -> assertThat(strReturn.contains("Email size can be between 5 and 64")).isTrue(),
                () -> assertThat(strReturn.contains("Password size can be between 2 and 64")).isTrue(),
                () -> assertThat(strReturn.contains("Standard e-mail structure - email_name@email_domain.top_lavel_domain (for example: paul@tradsystem.ru)")).isTrue()
        );
    }
    ...
    @Test
    @SneakyThrows
    void loginClient_NotExistClientLogin_ShouldReturnBadRequest_AndErrorWarnings_Test() {
        String requestBodyData = objectMapper.writeValueAsString(notExistingAuthRequest);
        ...
        assertThat(strReturn.contains("User name (email) not correct!")).isTrue();
    }
}
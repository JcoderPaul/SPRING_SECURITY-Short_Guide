...
class CardsControllerIT extends IntegrationTestBase {
    ...
    void getCardDetails_ShouldReturnOk_ReadAuth_Test() {
        ...
        List<CardReadDto> respDtoList = new ObjectMapper()
                .registerModule(new JavaTimeModule())
                .readValue(strRes, new TypeReference<List<CardReadDto>>() {});

        assertThat(respDtoList.size()).isGreaterThan(2);    // У данного клиента более 2-х карт в БД
    }

    @Test
    @SneakyThrows
    @WithUserDetails(value = "user3@test.com", userDetailsServiceBeanName = "clientDetailsService")
    void getCardDetails_ShouldReturnForbidden_HasNoReadAuth_Test() {
        mockMvc.perform(get("/api/myCards"))
                .andExpect(status().isForbidden())  // У данного клиента нет привилегий READ

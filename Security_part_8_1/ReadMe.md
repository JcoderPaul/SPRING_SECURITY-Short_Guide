...
AuthenticationEventListener.java. Однако мы с легкостью можем продолжать использовать технологию Servlet Filter-ов в нашем 
Spring приложении.

Spring Security - мощный фреймворк, который предоставляет комплексную службу безопасности для приложений Java. Одной из 
ее ключевых особенностей является возможность обработки задач безопасности с помощью цепочки фильтров (FilterChain). Эти 
...
пользовательские фильтры (custom filters). Ниже мы кратко рассмотрим процесс создания и интеграции пользовательских 
...
2. Пользовательские фильтры:
   ...
   - Пользовательские фильтры создаются путем реализации интерфейса Filter из пакета jakarta.servlet.
3. Добавление фильтров в цепочку:
   ...
   - Такие методы, как addFilterBefore(), addFilterAfter() и addFilterAt(), используются для указания позиции пользовательского фильтра в цепочке.
...
5. Реализация логики фильтра:
   - Основная логика пользовательского фильтра реализована в методе doFilter(), где фильтр обрабатывает запрос и ответ.
...
6. Управление контекстом безопасности (Security Context):
   - Security Context содержит сведения об аутентифицированном пользователе и управляется с помощью SecurityContextHolder.
...
Чтобы создать пользовательский фильтр, нам необходимо реализовать интерфейс Filter из пакета jakarta.servlet. Ниже приведен 
...
конфигурации с помощью объекта HttpSecurity. Вот как можно добавить CustomHeaderFilter перед 
UsernamePasswordAuthenticationFilter (применяем метод *.addFilterBefore()):
...
Практика будет облегченной - покажем только технику внедрения фильтров в цепочку безопасности - AppSecurityConfig.java.
...
- MyRequestValidationBeforeFilter.java - который будет внедрен перед UsernamePasswordAuthenticationFilter, основная логика 
...
- MyAuthoritiesLoggingAfterFilter.java - который будет внедрен после UsernamePasswordAuthenticationFilter фильтра и выведет 
...
- MyAuthoritiesLoggingAtFilter.java - данный фильтр просто логирует и выводит в консоль некий процесс. Для данного фильтра 
...
- MySecondRequestValidationBeforeFilter.java - так же внедряется перед UsernamePasswordAuthenticationFilter, но реализует чуть
более сложную логику, чем MyRequestValidationBeforeFilter.java. Мы обращаемся к remember-me cookie в request-е при его наличии 
и извлекаем userName из него см. комментарии к коду.

Для наглядности всего процесса мы сделали следующее:
- Шаг 1. - Добавили параметр в аннотацию @EnableWebSecurity(debug = true), что позволяет фиксировать происходящее.
- Шаг 2. - Добавили в настройки отображения логгера следующий параметр в application.yml (или строку application.properties): 
...
и задумывали см. AppSecurityConfig.java, так же мы видим то, что возвращают в лог самописные фильтры при первом обращении к 
странице логина:
...
запрос и сработал наш перехватчик фиксирующий процесс аутентификации - AuthenticationEventListener.java: 
